...
export default function Aside({
  responsiveContext, collapsed, showContent, chats, selectedChat, toggleAside, onSelectChatId
  ...
  return (
    <>
      <aside className={`${responsiveContext.width < FALLBACK_MOBILE_L_SCREEN_WIDTH ? `fixed top-0 left-0 h-full z-50 w-[260px] bg-[var(--theme-bg-chat-muted)] border-r border-[var(--theme-border-base)] transition-transform duration-300 ease-in-out ${collapsed ? "translate-x-[-260px]" : "translate-x-0"}` : ` relative h-full ${collapsed ? "w-[52px]" : "w-[260px]"} bg-[var(--theme-bg-chat-muted)] border-r border-[var(--theme-border-base)] transition-all duration-200 ease-in-out`}`}>
        ...
        {(responsiveContext.width < FALLBACK_MOBILE_L_SCREEN_WIDTH ? showContent : true) && (
            ...
            {(showContent && chats.length > 0) && (
              <>
                <button
                  ...
                  className="group select-none cursor-pointer flex items-center text-[14px] text-[var(--theme-text-caption)] pl-2 mt-4 mb-2"
                >
                  Your chats
                  {isChatListVisible ? (
                    <svg
      ...
      {(responsiveContext.width < FALLBACK_MOBILE_L_SCREEN_WIDTH && !collapsed) && (
        <div
          onClick={toggleAside}
          className="fixed inset-0 bg-[var(--theme-bg-dark)]/80 z-40"
        />
      )}
    </>
  )
